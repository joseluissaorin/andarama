...
docker compose exec -T ull360 node -e "
const Database = require('better-sqlite3');
const db = new Database('/data/ull360.db', { readonly: true });
db.exec(\"VACUUM INTO '/data/backup-tmp.db'\");
db.close();
"
docker compose cp "ull360:/data/backup-tmp.db" "$DEST/ull360-$STAMP.db"
docker compose exec -T ull360 rm -f /data/backup-tmp.db
