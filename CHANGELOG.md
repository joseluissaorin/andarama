...
- **Cupones canjeables.** Códigos de un solo uso con la forma `ANDA-XXXX-XXXX` que conceden un plan sin pasar por la pasarela: se generan por tandas en Administración (con lote, nota y descarga en CSV) y se canjean en la página Plan. El reparto es atómico, nunca rebajan a quien ya tiene un plan mejor y quedan en la auditoría. Los códigos promocionales de Clerk no servían: son descuentos sobre una suscripción, solo se crean a mano en su panel y el plan vitalicio es un pago único. Migración `0008_coupons`.
- **Ejecutable de escritorio.** Un solo fichero para macOS (Apple e Intel), Windows y Linux (x64 y ARM64) compilado con Bun: dentro van el servidor, el Studio, el visor y la documentación; los datos quedan en `~/Andarama` y al arrancar se abre el navegador. Mismo esquema y mismas migraciones que el self-host, SQLite integrado de Bun y tiempo real sobre sus WebSockets nativos. `pnpm build:desktop`.
